...
from .blocks import ConformerBlock2
...
class TFCB_CPq(nn.Module):
    def __init__(self, num_channel=64, attn_mask=None):
        super().__init__()

        self.time_conformer = ConformerBlock2(dim=num_channel, dim_head=num_channel//4, heads=4,
                                             conv_kernel_size=31, attn_dropout=0.2, ff_dropout=0.2)
        self.freq_conformer = ConformerBlock2(dim=num_channel, dim_head=num_channel//4, heads=4,
                                             conv_kernel_size=31, attn_dropout=0.2, ff_dropout=0.2)
        
        self.mhsa = nn.MultiheadAttention(embed_dim=num_channel, num_heads=4, batch_first=True)
    ...
    def forward(self, x_in):
        ...
        x_f = rearrange(x_in, 'b c t f -> (b t) f c')
        x_f = self.freq_conformer(x_f) + x_f
        x_t = rearrange(x_f, '(b t) f c -> (b f) t c', b=b)
        
        ## make x_v by Masked MHSA
        x_tq = rearrange(x_in, 'b c t f -> (b f) t c')
        # attn_mask_mhsa = torch.tril(torch.ones(x_tv.size(-2), x_tv.size(-2))).to(x_tv.device)
        # x_tv = self.mhsa(x_tv, x_tv, x_tv, attn_mask=attn_mask_mhsa)[0] + x_tv

        x_o = self.time_conformer.forward(x_t, q_given=x_tq, mask=self.attn_mask)+x_t   ## have to mask!!
        x_o = rearrange(x_o, '(b f) t c -> b c t f', b=b)
...
class TFCModel(nn.Module):
    def __init__(
        ...
        self.TSCBs = nn.Sequential(*[
            TFCB_CPq(num_channel=num_channel, attn_mask=attn_mask)
    ...
    def forward(self, x): 
        ...
        final_mag = torch.sqrt(final_real**2 + final_imag**2 + EPS)

        return dict(mag=final_mag, real=final_real, imag=final_imag, mask=mask, correction=complex_out)
